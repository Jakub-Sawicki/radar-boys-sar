...
ESP32_PORT = 3333
MEASUREMENTS = 75  # ile kroków i pomiarów
...
def main():
    ...
    for i in range(MEASUREMENTS):
        ...
        # Oblicz kąt metodą MLE SAR dla pojedynczego pomiaru
        try:
    ...
    # Opcjonalnie: porównaj z MLE na wszystkich danych naraz
    try:
        angle_mle_all = analyze_mle_method_all_data(measurements_data, verbose=True)
        print(f"[WYNIKI] MLE SAR na wszystkich danych: {angle_mle_all:.1f}°")
    except Exception as e:
        print(f"[ERROR] Błąd w MLE na wszystkich danych: {e}")
