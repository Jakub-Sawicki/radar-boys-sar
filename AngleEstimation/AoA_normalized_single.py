...
def MLE_sar_full_aperture_dual(Y, element_positions_mm, freq_hz):
    """
...
def analyze_mle_with_aperture_dual(measurements_data, data_index):
    """
    ...
    try:
        ...
        ch0 = np.array(measurements_data['ch0'][data_index])
        ch1 = np.array(measurements_data['ch1'][data_index]) * calibration_factor
        ch0_stack.append(ch0)
        ch1_stack.append(ch1)

        pos_mm = measurements_data['positions'][data_index] * 1000  # m → mm
        ...
        # Estymacja kąta
        estimated_angle = MLE_sar_full_aperture_dual(Y, element_positions_mm, SignalFreq)
...
if __name__ == "__main__":
    ...
    angle_sar_aperture_dual = analyze_mle_with_aperture_dual(measurements_data, data_index)
